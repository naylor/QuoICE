...
class candReceitas extends Generic {
    ...
    public function getCandReceitas($sqlAdicional = null, $params = null, $item = null, $itensPorPagina = null) {
        $bd = new database();
        ...
        if ($res) {
			...
			if ($res1) {
				...
				$l .= "<h1>Receita</h1><h2>".money($vl=isset($res[1]['valor'])?$res[1]['valor']:$res[0]['valor'])."</h2>";
				$l .= "</div>";
				$l .= "<div class=\"user-grf\">";
				$l .= "<img src=\"".VIEW."/file.php?grafico=".urlencode(serialize($this->getGrafico($res, 'receita')))."&time=".time()."\" />";
				$l .= "</div>";
				...
				$fala = $res1[0]['nome'].", receita de campanha no valor de $fala de reais";
